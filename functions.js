...
exports.view = (view_path, options, fn = undefined) => {
    ...
    return pug.renderFile(`${view_path}`, options, fn);
};
...
exports.output_boolean = (row, field = 'active') => {
    if(h.is_undef(row.getAttribute(field)))
    {
        console.log(`Unknown property ${field} in class ${h.get_class(row)}`);
 ...
 * @return {*}
 */
exports.output_desc = (row, field = 'body', length = 150) => {
    let body = row[field] ? row[field] : '';

    if(body)
    {
        return h.sprintf('%s...', 
            module.exports.strip_html(body).substr(0, length)
        );
    }
...
exports.output_label = (row, field = null, types = {}, value = null, attributes = {}) => {
    let title = String(value ? value : row[field]);
    let _type = 'default';
    let _title = 'none';
    attributes = module.exports.html_attributes(attributes);    
    ...
    return `<span class="label label-${_type}" ${attributes}>${_title}</span>`;
};
